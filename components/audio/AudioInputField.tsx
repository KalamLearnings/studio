 ...
 * 1. Audio Library - adding new audio assets (with category, tags, name)
 * 2. Activity Forms - instruction field with TTS (simpler, no metadata)
 *
 * Features:
 * - Mode toggle: Upload File / Generate from Text
 * - TTS with VoiceTagsInput and VoiceSelector
 * - File drag & drop with validation
 * - Audio preview/playback
 * - Template placeholder buttons (for activity forms)
 * - Optional: Category, Tags, Name fields (for audio library)
 */
...
export function AudioInputField({
  ...
  const handleVoiceChange = onVoiceChange ?? setInternalVoice;
      ...
      {(!isLibraryMode || inputMode === "tts") && (
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              type="button"
              variant={isLibraryMode ? "default" : "outline"}
              size={isLibraryMode ? "default" : "sm"}
              onClick={handleGenerateTTS}
              disabled={!ttsText.trim() || isGenerating}
              className={cn(
                !isLibraryMode && "text-purple-700 border-purple-300 hover:bg-purple-50"
              )}
            >
              {isGenerating ? (
                <>
